    ...
    for (let i = 0; i < this.data.length; i++) {
      ...
      const infoWindow = new google.maps.InfoWindow({
        content: '<a target="_blank" style="display: flex;flex-direction: column;align-items: center" href="/details">\n' +
          '<img src="' + this.url + this.data[i].imgs[0] + '" style="max-width: 80px;height: 80px;object-fit: cover">\n' +
          '<br>\n' +
          this.data[i].mapDetails.address +
          '</a>'
      });
      ...
      google.maps.event.addListener(this.otherMarkers, 'click', () => {
        localStorage.setItem('cart', JSON.stringify(this.data[i]));
